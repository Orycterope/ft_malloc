...
/*   Updated: 2017/03/05 20:47:42 by tvermeil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** If saving to the tables fails, returns NULL
*/
static t_buffer		*create_initial_buffer(size_t len)
{
	t_buffer		buf;
	t_table_entry	*entry;
	...
	buf.next_buf = NULL;
	buf.prev_buf = NULL;
	entry = save_data_to_tables((union u_entry_data)buf, ENTRY_BUFFER);
	if (entry)
		return (&entry->entry_data.buf);
	else
		return (NULL);
}
...
*/
static t_mapping	*create_mapping(size_t desired)
{
	t_mapping		mapping;
	t_table_entry	*entry;
	...
	if (mapping.map_addr == MAP_FAILED)
		return (NULL);
	mapping.buffers = create_initial_buffer(mapping.map_len);
	if (mapping.buffers == NULL)
	{
	...
	entry = save_data_to_tables((union u_entry_data)mapping, ENTRY_MAPPING);
	if (entry == NULL)
	{
		remove_entry_from_tables((union u_entry_data *)mapping.buffers);
		munmap(mapping.map_addr, mapping.map_len);
		return (NULL);
	}
	return (&entry->entry_data.map);
}
...
** If true it updates -best_ptr- to -current-
** If -best_map- points to NULL, -current- is considered better.
*/
static void			check_is_best(t_alloc_location *best,
	enum e_mapping_type type, t_mapping	*current_map, size_t alloc_size)
{
	...
	if (best->buf && current_map->shortest_free_buffer >= best->buf->len)
		return ;
	current_best_buf = find_best_free_buffer_in_mapping(current_map, alloc_size);
	...
	if (current_best_buf->len < best->buf->len)
	{
		best->map = current_map;
		best->buf = current_best_buf;
		return ;
...
t_alloc_location	find_best_mapping_for_size(size_t size)
{
	...
	t_table				*table;
	t_table_entry		*entry;
	t_alloc_location	best;

	type = get_mapping_type(size);
	ft_bzero(&best, sizeof(best));
	table = g_malloc_infos.tables;
	while (type != MAPPING_LARGE
			&& table != NULL && !(best.buf && best.buf->len == size))
	{
		entry = table->first_mapping_entry;
		while (entry != NULL && !(best.buf && best.buf->len == size))
		{
			check_is_best(&best, type, &entry->entry_data.map, size);
